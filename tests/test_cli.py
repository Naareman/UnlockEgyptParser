...
class TestParseArguments:
    ...
    def test_type_argument_multiple(self) -> None:
        ...
        with patch.object(sys, 'argv', ['unlockegypt', '-t', 'monuments', '-t', 'museums']):
            args = parse_arguments()
...
class TestPageType:
    ...
    def test_get_display_name(self) -> None:
        ...
        assert PageType.get_display_name(PageType.ARCHAEOLOGICAL_SITES) == "Archaeological Sites"
...
class TestMainFunction:
    ...
    def test_main_with_mock_researcher(self, tmp_path) -> None:
        ...
        with (
            patch.object(sys, 'argv', ['unlockegypt', '-o', str(output_file), '-m', '1']),
            ...
            mock_instance.research_all.return_value = []
            mock_instance.sites = []
            ...
            mock_instance.research_all.assert_called_once()
    ...
    def test_main_with_page_types(self, tmp_path) -> None:
        ...
        with (
            patch.object(sys, 'argv', ['unlockegypt', '-t', 'monuments', '-o', str(output_file)]),
            patch('unlockegypt.cli.SiteResearcher') as MockResearcher,
        ):
            mock_instance = MagicMock()
            mock_instance.__enter__ = MagicMock(return_value=mock_instance)
            ...
            # Verify research_all was called with correct page_types
            call_kwargs = mock_instance.research_all.call_args
            assert call_kwargs[1]['page_types'] == ['monuments']
    ...
    def test_main_verbose_logging(self, tmp_path) -> None:
        ...
        with (
            patch.object(sys, 'argv', ['unlockegypt', '-v', '-o', str(output_file)]),
            patch('unlockegypt.cli.SiteResearcher') as MockResearcher,
        ):
            mock_instance = MagicMock()
            mock_instance.__enter__ = MagicMock(return_value=mock_instance)
    ...
    def test_main_headless_option(self, tmp_path) -> None:
        ...
        with (
            patch.object(sys, 'argv', ['unlockegypt', '--no-headless', '-o', str(output_file)]),
            patch('unlockegypt.cli.SiteResearcher') as MockResearcher,
        ):
            mock_instance = MagicMock()
            mock_instance.__enter__ = MagicMock(return_value=mock_instance)
